...
This article explains the main TLS patterns across F5, WSO2, OpenShift Service Mesh 3 Gateway API ingress, and egress.
...
- `WSO2 -> Gateway API ingress`: TLS or mTLS
- `Istio internal`: mesh mTLS
- `Mesh -> egress gateway -> external`: TLS according to destination policy
...
flowchart TB
    ...
    ROOT --> IGWCERT["Gateway API ingress certificate"]
    ROOT --> EGWCERT["Optional egress gateway trust policy"]
...
- `Vault + cert-manager` can issue and renew Gateway API ingress gateway certs
...
- outbound trust to partners may be pinned or mediated through the egress layer
